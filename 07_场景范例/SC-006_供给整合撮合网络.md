# SC-006 · 供给整合撮合网络（标准范例）

> **视角**：业务/EM｜**主角角色**：`EU` ｜ **涉及平台**：EOS/EMS
...
EOS 供给运营对供给方(EU)与需求方进行智能撮合，形成稳定供给网络并结算服务费。
...
sequenceDiagram
        ...
        APP->>OS: 1. EU 供给能力登记(EOS)
        OS->>OAS/datalake: 2. EMS 供给资质审核
        APP->>OS: 3. 需求方发布需求
        OS->>OAS/datalake: 4. EOS 撮合匹配算法
        APP->>OS: 5. 撮合合约生成
        OS->>MS: 6. 履约与质量追踪
        APP->>OS: 7. 服务费结算 FMS
        OS->>OAS/datalake: 8. 供给网络图谱沉淀
```
...
- `eos.supply_capacities`
- `eos.match_orders`
- `eos.service_fees`
- `datalake.supply_network`

## 七、事件契约

复用统一事件类型（幂等键 = 事件类型 + 业务 ID，72h 超时，死信队列见 SC-013）：

`SupplyRegistered`, `MatchRequested`, `MatchCreated`, `ContractFulfilled`, `ServiceFeeSettled`
...
> **供给整合撮合网络** 场景完整走通了「业务/EM」下的角色→平台→管控→底座闭环，
验证了 A1.1 「EOS/EMS」协作的正确性与职责边界清晰性。
