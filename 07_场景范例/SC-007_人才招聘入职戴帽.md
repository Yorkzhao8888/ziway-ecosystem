# SC-007 · 人才招聘入职戴帽（标准范例）

> **视角**：职能/人力｜**主角角色**：`HU` ｜ **涉及平台**：HOS/HMS + EMS 合规
...
个人(HU)通过 Mate 注册，招聘入职后按岗位戴帽(HU→DU/PU/EU)，并完成 EMS 合规校验。

## 二、前置准备

- **角色**：`HU`（由四根身份 HU/EU/GU/OU 派生，戴帽机制见 SC-014）
...
sequenceDiagram
        ...
        APP->>OS: 1. HU 注册 Mate(HOS)
        OS->>OAS/datalake: 2. 简历/能力建档 HMS
        APP->>OS: 3. 企业发起招聘
        OS->>OAS/datalake: 4. 面试入职
        APP->>OS: 5. 戴帽 HU→目标角色(HMS 更新 hats)
        OS->>MS: 6. EMS 合规校验(身份/资质)
        APP->>OS: 7. 入职生效(tenant 权限)
        OS->>OAS/datalake: 8. 人力数据分析
```
...
`TalentRegistered`, `ResumeSubmitted`, `Recruited`, `HatGranted`, `ComplianceChecked`
...
> **人才招聘入职戴帽** 场景完整走通了「职能/人力」下的角色→平台→管控→底座闭环，
验证了 A1.1 「HOS/HMS + EMS 合规」协作的正确性与职责边界清晰性。
