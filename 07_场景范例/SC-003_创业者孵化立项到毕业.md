# SC-003 · 创业者孵化立项到毕业（标准范例）

> **视角**：产研/孵化｜**主角角色**：`SU` ｜ **涉及平台**：SOS/SMS + EMS
...
创业者(SU)在孵化园(SOS)提交立项，经 SMS 评审入孵、获得 EMS 供给扶持，达到毕业标准后转出。

## 二、前置准备

- **角色**：`SU`（由四根身份 HU/EU/GU/OU 派生，戴帽机制见 SC-014）
...
sequenceDiagram
        ...
        APP->>OS: 1. SU 提交孵化立项(SOS)
        OS->>OAS/datalake: 2. SMS 立项评审
        APP->>OS: 3. 入孵建档与资源分配
        OS->>OAS/datalake: 4. 孵化期辅导/里程碑考核
        APP->>OS: 5. EMS 供给/场地扶持
        OS->>MS: 6. 毕业评审(SMS)
        APP->>OS: 7. 毕业转出(生成新 EU 或 DU)
        OS->>OAS/datalake: 8. 孵化成效回流 VMS
```
...
- `sos.incubation_projects`
- `sos.milestones`
- `sos.graduation_records`
- `datalake.incubation_metrics`

## 七、事件契约

复用统一事件类型（幂等键 = 事件类型 + 业务 ID，72h 超时，死信队列见 SC-013）：

`ProjectSubmitted`, `ProjectApproved`, `MilestoneReached`, `GraduationReviewed`, `Graduated`
...
> **创业者孵化立项到毕业** 场景完整走通了「产研/孵化」下的角色→平台→管控→底座闭环，
验证了 A1.1 「SOS/SMS + EMS」协作的正确性与职责边界清晰性。
