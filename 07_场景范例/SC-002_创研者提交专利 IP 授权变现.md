# SC-002 · 创研者提交专利 IP 授权变现（标准范例）

> **视角**：产研/TM｜**主角角色**：`PU` ｜ **涉及平台**：POS/PMS → Lab
...
创研者(PU)在 Lab 提交技术/IP/专利，经 PMS 审核后进入 POS 授权市场，被 DM 产品方授权使用并分账。

## 二、前置准备

- **角色**：`PU`（由四根身份 HU/EU/GU/OU 派生，戴帽机制见 SC-014）
...
sequenceDiagram
        ...
        APP->>OS: 1. PU 注册/戴帽进入 Lab
        OS->>OAS/datalake: 2. 提交专利/IP/配方(POS)
        APP->>OS: 3. PMS 知识产权审核
        OS->>OAS/datalake: 4. IP 上架授权市场
        APP->>OS: 5. DM 产品方发起授权(调用 POS)
        OS->>MS: 6. 授权合同签署与计费
        APP->>OS: 7. 授权费分账至 PU
        OS->>OAS/datalake: 8. 授权记录沉淀 datalake
```
...
- `pos.intellectual_properties`
- `pos.ip_licenses`
- `pos.royalty_settlements`
- `datalake.ip_metrics`
...
`IPSubmitted`, `IPApproved`, `IPListed`, `LicenseRequested`, `LicenseSigned`, `RoyaltySettled`
...
> **创研者提交专利 IP 授权变现** 场景完整走通了「产研/TM」下的角色→平台→管控→底座闭环，
验证了 A1.1 「POS/PMS → Lab」协作的正确性与职责边界清晰性。
